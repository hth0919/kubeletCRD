...
(underspec'd currently) to submit a new manifest.`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if outOfClusterConfig = kubeletFlags.KubeConfig; len(outOfClusterConfig)>0 {
				klog.Infoln(outOfClusterConfig)
			}
...
func run(s *options.KubeletServer, kubeDeps *kubelet.Dependencies, featureGate featuregate.FeatureGate, stopCh <-chan struct{}) (err error) {
	...
	case kubeDeps.KubeClient == nil, kubeDeps.EventClient == nil, kubeDeps.HeartbeatClient == nil:
		...
		clientConfig, err = clientcmd.BuildConfigFromFlags("",outOfClusterConfig)
