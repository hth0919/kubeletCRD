...
func (t *SysctlUpgradeTest) Test(f *framework.Framework, done <-chan struct{}, upgrade UpgradeType) {
	...
	case MasterUpgrade, ClusterUpgrade:
		...
		pod, err := f.ClientSet.KetiV1().Pods(t.validPod.Namespace).Get(t.validPod.Name, metav1.GetOptions{})
	...
	pod, err := f.ClientSet.KetiV1().Pods(t.invalidPod.Namespace).Get(t.invalidPod.Name, metav1.GetOptions{})
