...
func (mc *basicMirrorClient) CreateMirrorPod(pod *v1.Pod) error {
	...
	apiPod, err := mc.apiserverClient.KetiV1().Pods(copyPod.Namespace).Create((*ketiv1.Pod)(&copyPod))
...
func (mc *basicMirrorClient) DeleteMirrorPod(podFullName string, uid *types.UID) (bool, error) {
	...
	if err := mc.apiserverClient.KetiV1().Pods(namespace).Delete(name, &metav1.DeleteOptions{GracePeriodSeconds: &GracePeriodSeconds, Preconditions: &metav1.Preconditions{UID: uid}}); err != nil {
