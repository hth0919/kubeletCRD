...
func (m *manager) syncPod(uid types.UID, status versionedPodStatus) {
	...
	kpod, err := m.kubeClient.KetiV1().Pods(status.podNamespace).Get(status.podName, metav1.GetOptions{})
	pod := (*v1.Pod)(kpod)
	...
	if m.canBeDeleted(pod, status.status) {
		...
		err = m.kubeClient.KetiV1().Pods(pod.Namespace).Delete(pod.Name, deleteOptions)
		if err != nil {
