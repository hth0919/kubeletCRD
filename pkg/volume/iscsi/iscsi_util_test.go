...
func TestExtractDeviceAndPrefix(t *testing.T) {
	...
	mountPrefix := "/var/lib/keti-kubelet/plugins/kubernetes.io/iscsi/iface-default/" + devicePath
	lun := "-lun-0"
...
func TestExtractIface(t *testing.T) {
	...
	iface, found := extractIface("/var/lib/keti-kubelet/plugins/kubernetes.io/iscsi/iface-" + ifaceName + "/" + devicePath)
	if !found || iface != ifaceName {
		t.Errorf("extractIface: expected %s and %t, got %s and %t", ifaceName, true, iface, found)
	}
	iface, found = extractIface("/var/lib/keti-kubelet/plugins/kubernetes.io/iscsi/" + devicePath)
	if found || iface != "" {
